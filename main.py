...
def get_garden_details(garden_id: int, db: Session = Depends(get_db)):
    ...
    recommendation_text = latest_update.recommendation if latest_update else ""
    words = recommendation_text.split()
    recommendation_show_more = len(words) > 10
    truncated_recommendation = " ".join(words[:10])
    ...
    return {
        ...
        "recommendation_show_more": recommendation_show_more,
        "has_pests": latest_update.has_pests if latest_update else False,
        "has_weeds": latest_update.has_weeds if latest_update else False,
        "is_overwatered": latest_update.is_overwatered if latest_update else False,
        "is_underwatered": latest_update.is_underwatered if latest_update else False,
