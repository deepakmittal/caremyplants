...
def get_garden_details(garden_id: int, db: Session = Depends(get_db)):
    ...
    short_recommendation = (" ".join(recommendation.split()[:10]) + "...") if recommendation and len(recommendation.split()) > 10 else recommendation
    ...
    return {
        ...
        "weed_presence": latest_update.weed_presence if latest_update else None,
        "flowering_status": latest_update.flowering_status if latest_update else None,
