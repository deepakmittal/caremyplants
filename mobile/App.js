...
const PlantCard = ({ plant, index, gardenStatus }) => {
  return (
    <View style={[styles.card, { marginLeft: index === 0 ? theme.spacing.margin : 16, width: CARD_WIDTH - 20 }]}>
      {plant.image_url ? (
        ...
        <View style={[styles.cardImage, { height: 220, justifyContent: 'center', alignItems: 'center' }]}>
          ...
          <Text style={{ marginTop: 12, color: theme.colors.outline, fontFamily: 'PlusJakartaSans_600SemiBold' }}>Extracting cutout...</Text>
        </View>
      ) : (
        <View style={[styles.cardImage, { height: 220, justifyContent: 'center', alignItems: 'center', backgroundColor: theme.colors.surfaceContainerHigh }]}>
          ...
          <Text style={{ marginTop: 12, color: theme.colors.outline, fontFamily: 'PlusJakartaSans_600SemiBold' }}>Image unavailable</Text>
        </View>
      )}
      <View style={styles.cardContent}>
            ...
            <View style={styles.analysisContainer}>
              ...
              <View style={{ flexShrink: 1 }}>
                ...
                {selectedGarden.upload_commentry && (
                  <Text style={[styles.analysisText, { marginTop: 4, fontFamily: 'PlusJakartaSans_400Regular', color: theme.colors.outline }]}>
                    {selectedGarden.upload_commentry}
                  </Text>
                )}
              </View>
          ...
          >
            {selectedGarden.plants.length > 0 ? (
              selectedGarden.plants.map((plant, index) => (
                <PlantCard key={plant.id} plant={plant} index={index} gardenStatus={selectedGarden.status} />
              ))
            ) : (
